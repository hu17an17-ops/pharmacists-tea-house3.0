...
function getPreparationMinutesByCups(cups) {
  cups = Math.max(0, Math.floor(num(cups)));

  if (cups <= 1) return 10;
  if (cups <= 5) return 20;
  if (cups <= 9) return 30;
  if (cups <= 12) return 40;
  if (cups <= 15) return 50;
  return 60;
}

function getDrinkCountFromItems(items) {
  return (items || []).reduce((sum, item) => {
    return sum + Math.max(0, Math.floor(num(item.quantity)));
  }, 0);
}
...
async function createOrder(b) {
  ...
  // 依訂購總杯數決定最早取餐時間，後端再次驗證避免前端被繞過。
  const drinkCount = getDrinkCountFromItems(items);
  const preparationMinutes = getPreparationMinutesByCups(drinkCount);

  const pickupMatch2 = pickup.match(/^(\d{1,2})\/(\d{1,2})\s+(\d{1,2}):(\d{2})$/);
  if (pickupMatch2) {
    const pickupMonth = Number(pickupMatch2[1]);
    const pickupDay = Number(pickupMatch2[2]);
    const pickupHour = Number(pickupMatch2[3]);
    const pickupMinute = Number(pickupMatch2[4]);
    // Render 等伺服器環境可能使用 UTC；這裡統一以台灣時間（UTC+8）判斷。
    const now = new Date();
    const taiwanParts = new Intl.DateTimeFormat('en-CA', {
      timeZone: 'Asia/Taipei',
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
    ...
    const part = type => taiwanParts.find(x => x.type === type)?.value || '';
    const taiwanYear = Number(part('year'));
    const taiwanMonth = Number(part('month'));
    const taiwanDay = Number(part('day'));
    const taiwanHour = Number(part('hour'));
    const taiwanMinute = Number(part('minute'));

    const pickupTaiwanAsUtc = Date.UTC(pickupYear, pickupMonth - 1, pickupDay, pickupHour, pickupMinute);

    // 今天的訂單：依杯數限制最早取餐時間；最晚 22:00。
    if (pickupYear === taiwanYear && pickupMonth === taiwanMonth && pickupDay === taiwanDay) {
      const nowTaiwanAsUtc = Date.UTC(taiwanYear, taiwanMonth - 1, taiwanDay, taiwanHour, taiwanMinute);
      const earliestTaiwanAsUtc = nowTaiwanAsUtc + preparationMinutes * 60000;
      if (pickupTaiwanAsUtc < earliestTaiwanAsUtc) {
        throw new Error(`目前共 ${drinkCount} 杯，最早需 ${preparationMinutes} 分鐘後取餐，請重新選擇取餐時間。`);
      }
    } else {
      // 預訂未來營業日：以營業開始 11:30 為製作起算點，
      // 同樣依杯數決定最早取餐時間，避免週一、週二預訂時全部都從 11:30 開始。
      const openingTaiwanAsUtc = Date.UTC(pickupYear, pickupMonth - 1, pickupDay, 11, 30);
      const earliestFutureTaiwanAsUtc = openingTaiwanAsUtc + preparationMinutes * 60000;
      if (pickupTaiwanAsUtc < earliestFutureTaiwanAsUtc) {
        throw new Error(`目前共 ${drinkCount} 杯，該取餐日最早需於 ${preparationMinutes} 分鐘製作後取餐，請重新選擇取餐時間。`);
      }
    }

    // 所有取餐日最晚 22:00。
    const lastPickupTaiwanAsUtc = Date.UTC(pickupYear, pickupMonth - 1, pickupDay, 22, 0);
    if (pickupTaiwanAsUtc > lastPickupTaiwanAsUtc) {
      throw new Error('最晚取餐時間為 22:00。');
    }
  }
