...
async function createOrder(b) {
  ...
  /*
   * Supabase 是正式訂單唯一來源。
   * Render 的本機檔案系統不是永久儲存，因此不能讓「只寫本機」的訂單
   * 被當成成功訂單；否則重新部署／重啟後後台會消失。
   */
  if (!hasSupabase()) {
    throw new Error('訂單系統尚未連接 Supabase，為避免訂單遺失目前暫停下單。');
  }
  ...
  try {
    ...
    if (!Array.isArray(saved) || !saved[0]) {
      throw new Error('Supabase 沒有回傳已建立的訂單資料。');
    }

    supabaseSaved = true;
  } catch (e) {
    console.error('Supabase 寫入訂單失敗：', e);
    throw new Error('訂單無法儲存到雲端資料庫，請稍後再試。');
  }

  // 本機只做備份，不是正式來源；失敗不影響已存在 Supabase 的訂單。
  try {
    const local = readOrders();
    local.unshift(order);
    writeOrders(local);
  } catch (e) {
    console.error('本機訂單備份失敗（Supabase 已成功）：', e.message);
  }
  ...
  return {
    ...
    lineAdminNotification: adminLineNotified,
    customerLineNotification: customerLineNotified
  };
}
  ...
  try {
    ...
    if (
      ...
      if (!LINE_ACCESS_TOKEN() || !LINE_ADMIN_USER_ID()) {
        return send(res, 503, {
          ok: false,
          linePush: false,
          message: 'LINE_CHANNEL_ACCESS_TOKEN 或 LINE_ADMIN_USER_ID 尚未設定。'
        });
      }

      try {
        await linePush(
          '🔔 LINE Push 測試成功\n藥師的私房紅茶訂單系統已可發送官方 LINE 通知。',
          LINE_ADMIN_USER_ID()
        );
        ...
        return send(res, 200, {
          ok: true,
          linePush: true,
          message: 'LINE Push 已送出，請檢查官方 LINE 收件帳號。'
        });
      } catch (e) {
        ...
        return send(res, 502, {
          ok: false,
          linePush: false,
          message: e.message || 'LINE Push 失敗'
        });
      }
    }
    ...
    if (req.method === 'GET' && u.pathname === '/admin') {
      ...
      try {
        ...
        // 後台正式資料來源固定為 Supabase，確保與客人下單使用同一資料庫。
