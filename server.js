...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && u.pathname === '/admin') {
      ...
      let rows = [];
      try { rows = hasSupabase() ? await getOrders() : readOrders(); }
      catch (e) { console.error('後台讀取訂單失敗', e); }
      return send(res, 200, adminHtml(rows, key), 'text/html; charset=utf-8');
    }

    if (req.method === 'DELETE' && /^\/api\/admin\/orders\/[^/]+$/.test(u.pathname)) {
      if (u.searchParams.get('key') !== adminKey()) return send(res, 401, { ok:false, message:'Unauthorized' });
      const n = decodeURIComponent(u.pathname.split('/').pop());
      if (!resolveOrderNumber(n)) return send(res, 400, { ok:false, message:'訂單編號格式錯誤' });
      ...
      try {
        ...
        return send(res, 200, { ok:true, orderId:makeOrderId(resolveOrderNumber(n)), orderNumber:resolveOrderNumber(n), deletedFromSupabase:Array.isArray(deleted)?deleted.length:0, deletedFromLocalBackup:local });
      } catch (e) {
