...
async function linePush(text, to) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (!r.ok) {
        throw new Error(data?.message || data?.details?.[0]?.message || `LINE API ${r.status}`);
      }
  ...
  throw lastError || new Error('LINE Push 失敗');
}
...
async function getNextOrderNumberUnlocked() {
  ...
  const base = Number(dateKey) * 100;
  let latest = 0;

  // 不再用 gte/lt 篩選 order_number，因為舊資料曾經是 T16B86B2 這類字串，
  // 會讓 Supabase 的型別／排序造成錯誤。改成抓目前訂單後，在程式內嚴格解析 PTH 編號。
  if (hasSupabase()) {
    try {
      const rows = await supabase(
        'orders?select=order_number,created_at&limit=5000'
      );
      for (const row of Array.isArray(rows) ? rows : []) {
        const raw = String(row.order_number ?? '').trim().toUpperCase();
        if (isValidPthOrderNumber(raw) && raw.slice(3, 11) === dateKey) {
          const seq = Number(raw.slice(11));
          if (seq >= 1 && seq <= 99) latest = Math.max(latest, seq);
        }
      }
    } catch (e) {
      console.error('讀取 Supabase 訂單流水號失敗，改查本機備份：', e.message);
    }
  }

  for (const row of readOrders()) {
    const raw = String(row.orderNumber ?? row.order_number ?? '').trim().toUpperCase();
    if (isValidPthOrderNumber(raw) && raw.slice(3, 11) === dateKey) {
      const seq = Number(raw.slice(11));
      if (seq >= 1 && seq <= 99) latest = Math.max(latest, seq);
    }
  ...
  if (latest >= 99) {
    throw new Error('今日訂單編號已超過 99 筆，請聯絡管理員。');
  }
...
function isValidPthOrderNumber(value) {
  return /^PTH\d{10}$/.test(String(value ?? '').trim().toUpperCase());
}

function taiwanDateKeyFromIso(value) {
...
async function repairLegacyOrderNumbers(rows) {
  if (!hasSupabase() || !Array.isArray(rows) || !rows.length) return rows;

  const groups = new Map();
  for (const row of rows) {
    const dateKey = taiwanDateKeyFromIso(row.created_at);
    if (!dateKey) continue;
  ...
  for (const [dateKey, group] of groups) {
    ...
    for (const row of group) {
      const raw = String(row.order_number ?? '').trim().toUpperCase();
      if (isValidPthOrderNumber(raw) && raw.slice(3, 11) === dateKey) {
        used.add(Number(raw.slice(11)));
      }
    }
    usedByDate.set(dateKey, used);
  }
  ...
  for (const [dateKey, group] of groups) {
    ...
    const legacy = group
      .filter(row => {
        const raw = String(row.order_number ?? '').trim().toUpperCase();
        return !isValidPthOrderNumber(raw) || raw.slice(3, 11) !== dateKey;
      })
    ...
    for (const row of legacy) {
      ...
      const newNumber = Number(`${dateKey}${String(seq).padStart(2, '0')}`);
      // PTH + YYYYMMDD + 2-digit sequence，與目前店內編號規則一致。
      try {
        const updated = await supabase(
          `orders?id=eq.${encodeURIComponent(row.id)}`,
          { method: 'PATCH', body: { order_number: newNumber } }
        );
        const saved = Array.isArray(updated) ? updated[0] : null;
        row.order_number = saved?.order_number ?? newNumber;
        used.add(seq);
        repaired.push({ old: row.order_number, newNumber });
      } catch (e) {
        console.error('修復舊訂單編號失敗：', e.message);
      }
    }
  }
...
async function createOrder(b) {
  ...
  /*
   * 正式訂單一律以 Supabase 為唯一成功條件。
   * 不再出現「畫面顯示成功，但後台其實沒有資料」的假成功。
   * Supabase 成功後才寫本機備份，作為額外保留。
   */
  if (!hasSupabase()) {
    throw new Error('Supabase 尚未設定，為避免訂單遺失，本次不接受下單。');
  }
  ...
  try {
    ...
    console.error('Supabase 寫入訂單失敗：', supabaseError);
    throw new Error(`訂單尚未建立，資料庫儲存失敗：${supabaseError}`);
  }
  ...
  return {
    ...
    supabaseOrderId: Array.isArray(saved)
      ? saved[0]?.id
      : saved?.id || null,
    storage: supabaseSaved ? 'supabase+local-backup' : 'none',
    supabaseError: supabaseSaved ? null : (supabaseError || null),
    lineAdminNotified: adminLineNotified,
    lineCustomerNotified: customerLineNotified
  };
}
  ...
  try {
    ...
    if (
      ...
      return send(res, 200, {
        ...
        version: '20260830-line-order-final-v5',
        supabase: hasSupabase(),
        line: Boolean(LINE_ACCESS_TOKEN()),
        lineAdminReady: Boolean(
          LINE_ACCESS_TOKEN() &&
          LINE_ADMIN_USER_ID()
        ),
        webhook: Boolean(
    ...
    if (
      ...
      u.pathname === '/api/line/test'
    ) {
      if (u.searchParams.get('key') !== adminKey()) {
        return send(res, 401, { ok: false, message: 'Unauthorized' });
      }
      if (!LINE_ACCESS_TOKEN() || !LINE_ADMIN_USER_ID()) {
        return send(res, 503, { ok: false, message: 'LINE_ACCESS_TOKEN 或 LINE_ADMIN_USER_ID 尚未設定' });
      }
      try {
        await linePush(
          '🔔 LINE Push 測試成功\n藥師的私房紅茶\n這代表官方 LINE 可以收到新訂單通知。',
          LINE_ADMIN_USER_ID()
        );
        return send(res, 200, { ok: true, message: 'LINE Push 測試已送出', recipientConfigured: true });
      } catch (e) {
        console.error('LINE Push 測試失敗：', e);
        return send(res, 502, { ok: false, message: e.message || 'LINE Push 測試失敗' });
      }
    }
    ...
    if (req.method === 'GET' && u.pathname === '/admin') {
      ...
      try {
        if (!hasSupabase()) throw new Error('Supabase 尚未設定');
        // 後台正式資料來源固定為 Supabase；不能用 num() 過濾，否則舊版 T16... 訂單會直接消失。
        rows = await getOrders();
      } catch (e) {
        console.error('讀取 Supabase 訂單錯誤，改讀本機備份：', e);
        rows = localRows;
      }

      // 一次性修復舊版 T16...／重複編號；修復後重新讀取，確保後台看到的就是資料庫實際編號。
      if (hasSupabase() && rows.length) {
        try {
          await repairLegacyOrderNumbers(rows);
          rows = await getOrders();
        } catch (e) {
          console.error('舊訂單編號自動修復失敗：', e.message);
        }
      }
