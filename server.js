...
async function createOrder(b) {
  ...
  if (!/^PTH\d{10}$/.test(makeOrderId(orderNumber))) {
...
function isPublicPthNumber(value){
  const raw = String(value ?? '').trim().toUpperCase();
  if (/^PTH\d{10}$/.test(raw)) return true;
  return /^20\d{8}$/.test(raw);
}

function adminDisplayIds(rows){
  const map = new Map();
  const groups = new Map();

  for (const r of rows) {
    const n = String(r.order_number ?? '').trim();
    if (isPublicPthNumber(n)) {
      const normalized = /^PTH\d{10}$/.test(n) ? n.toUpperCase() : `PTH${n}`;
      map.set(`${n}|${r.created_at ?? ''}`, normalized);
      continue;
    }
  ...
  for (const [day, dayRows] of groups) {
    dayRows.sort((a,b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
    const used = new Set();

    for (const r of dayRows) {
      const raw = String(r.order_number ?? '').trim();
      if (isPublicPthNumber(raw)) {
        const digits = raw.startsWith('PTH') ? raw.slice(3) : raw;
        used.add(Number(digits.slice(-2)));
      }
    }
    ...
    for (const r of dayRows) {
      const raw = String(r.order_number ?? '').trim();
      if (isPublicPthNumber(raw)) continue;
      while (used.has(next)) next += 1;
      const key = `${raw}|${r.created_at ?? ''}`;
      map.set(key, `PTH${day}${String(next).padStart(2,'0')}`);
      used.add(next);
      next += 1;
    }
  }
...
function adminHtml(rows,key){
  ...
  const displayIds = adminDisplayIds(rows);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && u.pathname === '/admin') {
      ...
      let rows = [];
      try {
        rows = hasSupabase() ? await getOrders() : localAsRows();
      } catch (e) {
        console.error('讀取訂單錯誤', e);
        rows = localAsRows();
      }
