...
// Multiple order-notification recipients: comma/newline separated LINE user/group/room IDs.
const LINE_ADMIN_TARGETS = () => String(process.env.LINE_ADMIN_TARGETS || '').trim();
const getLineAdminTargets = () => {
  const configured = LINE_ADMIN_TARGETS();
  if (configured) return configured.split(/[\s,]+/).map(v => v.trim()).filter(Boolean);
  const legacy = LINE_ADMIN_USER_ID();
...
async function createOrder(b) {
  ...
  // Notify every configured admin target (personal LINE, group, or room).
  const adminTargets = getLineAdminTargets();
  for (const target of adminTargets) {
    try {
      await linePush(buildOrderMessage(order), target);
      adminLineNotified = true;
    } catch (e) {
      console.error(`官方 LINE 訂單通知失敗（${target}）：`, e.message);
    }
  }
