...
function updatePickupTimeOptions() {
  ...
  for (let offset = 0; offset <= 7; offset++) {
    ...
    if (isToday) {
      ...
      firstTime = new Date(now.getTime() + getPreparationMinutes() * 60000);
      const remainder = firstTime.getMinutes() % 10;
      if (remainder !== 0) {
        firstTime.setMinutes(firstTime.getMinutes() + (10 - remainder));
      }
      firstTime.setSeconds(0, 0);
      if (firstTime < openingTime) firstTime = new Date(openingTime);
    } else {
      // 預訂未來營業日（包含週一、週二預訂週三之後）：
      // 不能直接從 11:30 開始，仍要依訂購杯數計算製作時間。
      // 以營業開始時間 11:30 為製作起算點。
      firstTime = new Date(
        openingTime.getTime() +
        getPreparationMinutes() * 60000
      );
      ...
      if (remainder !== 0) {
        firstTime.setMinutes(
          firstTime.getMinutes() + (10 - remainder)
        );
      }
      firstTime.setSeconds(0, 0);
    }
    ...
    for (let time = new Date(firstTime); time <= lastPickupTime; time.setMinutes(time.getMinutes() + 10)) {
      const option = document.createElement("option");
      option.value = `${date.getMonth() + 1}/${date.getDate()} ${pad(time.getHours())}:${pad(time.getMinutes())}`;
      option.textContent = `${formatDate(date).trim()} ${pad(time.getHours())}:${pad(time.getMinutes())}`;
  ...
  if (oldValue) {
    const option = Array.from(select.options).find(item => item.value === oldValue);
    if (option) select.value = oldValue;
  }
