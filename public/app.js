...
function updatePickupTimeOptions() {
  ...
  for (let offset = 0; offset <= 7; offset++) {
    ...
    if (isToday) {
      ...
      const remainder = firstTime.getMinutes() % 5;
      if (remainder !== 0) {
        firstTime.setMinutes(firstTime.getMinutes() + (5 - remainder));
      }
    ...
    for (let time = new Date(firstTime); time <= lastPickupTime; time.setMinutes(time.getMinutes() + 5)) {
      const option = document.createElement("option");
      option.value = `${date.getMonth() + 1}/${date.getDate()} ${pad(time.getHours())}:${pad(time.getMinutes())}`;
      option.textContent = `${pad(time.getHours())}:${pad(time.getMinutes())}`;
