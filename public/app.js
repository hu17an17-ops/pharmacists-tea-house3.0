...
function showOrderSuccess(orderId, total, pickupDateTime) {
  // V4: do not rely on <dialog> or any element in document flow.
  // Create a fixed viewport overlay so it can never appear underneath the menu.
  const old = document.querySelector("#orderSuccessOverlay");
  if (old) old.remove();
  ...
  // 清掉舊版部署可能殘留的「訂單已送出」區塊，避免成功視窗下方再出現第二份結果。
  document.querySelectorAll("#result, .order-result, [data-order-result]").forEach(el => el.remove());
  document.querySelectorAll(".success").forEach(el => {
    if (String(el.textContent || "").includes("訂單已送出")) el.remove();
  });
  ...
  overlay.innerHTML = `
    <div class="v4-card">
      <section class="v4-summary">
        <h2>訂單已送出！</h2>
        <div class="v4-line"><span>訂單編號：</span><strong>${escapeHtml(orderId || "—")}</strong></div>
        <div class="v4-line"><span>取餐時間：</span><strong>${escapeHtml(pickupDateTime || "—")}</strong></div>
        <div class="v4-line"><span>合計：</span><strong>${escapeHtml(money(Number(total) || 0))}</strong></div>
      </section>
      <section class="v4-reminder">
        <h3>📸 請立即截圖或保存此訂單畫面！</h3>
        <div class="v4-divider"></div>
        <p>到店取餐時，請向店員報「<strong>手機後三碼及姓氏</strong>」，以便核對餐點並現場付款（<strong>現金</strong>）。</p>
        <div class="v4-divider"></div>
        <p class="v4-warning">⚠️ 為避免忘記訂單編號，建議現在就將此畫面截圖或保存。</p>
      </section>
      <button type="button" class="v4-close" id="v4SuccessClose">知道了</button>
    </div>`;
  ...
  overlay.querySelector("#v4SuccessClose").addEventListener("click", close);
